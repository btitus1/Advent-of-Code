...
#define GSZ    128
#define TEND    4096
#define INPUT_FILE "datafile.txt"
#define WITH_VIS
...
int main() {
    ...
    t = route(0, 1, 0, gw - 2, gh - 1);
    p1 = t;
    t = route(t, gw - 2, gh - 1, 1, 0);
    t = route(t, 1, 0, gw - 2, gh - 1);
    p2 = t;

    vis24_end();

    printf("Minutes to reach goal and avoid blizzards : %d\n", p1);
    printf("Minutes to reach goal, go back to start and back to goal : %d\n", p2);
    return 0;
}
