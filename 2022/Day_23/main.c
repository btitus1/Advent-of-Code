...
int main() {
    ...
    for (y=PAD; fgets(&grid[y][PAD], GSZ-PAD, fp); y++)
        assert(y < GSZ);
    for (y=0; y<GSZ; y++)
        for (x=0; x<GSZ; x++)
            grid[y][x] = grid[y][x] == '#';
    ...
    printf("Number of empty ground tiles : %d\n", p1);
    printf("Number of rounds before an elf doesn't move : %d\n", i+1);
    return 0;
}
