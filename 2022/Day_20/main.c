...
)
{
    ...
    for (size_t n = 0; n < mix_times; n++)
    {
        ...
        for (int64_t i = 0; i < value_count; i++)
        {
            ...
            // we are taking the modulo by the list length minus 1 (not the list length itself).
...
{
    // Open the input file
    FILE *input = fopen("datafile.txt", "rt");
    char line[16];
    ...
    for (int64_t i = 0; i < value_count; i++)
    {
        values_p2[i].number *= DECRYPTION_KEY;
    }
    ...
    // Print the answers
    printf("Sum of the three numbers that form the grove coordinates: %ld\n", solution_p1);
    printf("After applying decryption key & mixing 10 times : %ld\n", solution_p2);
