...
/* number equals score for result */
typedef enum {
    EAST = 0,
    SOUTH = 1,
    WEST = 2,
...
static void map_find_start(grid_t *m, player_t *p);
static void map_print(grid_t *m);
static void map_file(grid_t *m);
static void player_turn(player_t *p, char d);
static int player_move(player_t *p, grid_t *m, int s);
static void player_move_right(player_t *p, grid_t *m, int s);
static void player_move_left(player_t *p, grid_t *m, int s);
static void player_move_up(player_t *p, grid_t *m, int s);
static void player_move_down(player_t *p, grid_t *m, int s);
static int wrap_r_to_l(const player_t *p, const grid_t *m, size_t *new_x);
static int wrap_l_to_r(const player_t *p, const grid_t *m, size_t *new_x);
static int wrap_b_to_t(const player_t *p, const grid_t *m, size_t *new_y);
static int wrap_t_to_b(const player_t *p, const grid_t *m, size_t *new_y);
...
int main() {
    ...
    printf("Start position: x=%lu y=%lu f=%c\n", player.pos.x, player.pos.y, dir[player.facing]);
    printf("Map size: max_x=%lu max_y=%lu\n", map.max.x, map.max.y);
